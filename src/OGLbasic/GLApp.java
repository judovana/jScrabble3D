 ...
 * subclass will define more elaborate render() mouseMove() functions, etc.
 * <P>
...
public class GLApp {
    ...
    public static String inititalDisplayMode = "autodetect";
    ...
    /**
     * Runs the application. Calls init() functions to setup OpenGL, input and
     * display. Runs the main loop of the application, which handles mouse and
     * keyboard input.
     * <P>
     * Calls: init() and mainLoop(). <BR>
     * mainLoop() calls: render(), mouseMove(), mouseDown(), mouseUp(),
     * keyDown(), keyUp()
     */
    public void run() {
        double delay = 0d;
     ...
     * <P>
     * Currently only works with Mouse.setGrabbed(true) (see initInput()) This
     * means the native cursor is hidden and app has to draw its own.
     */
    ...
    /**
     * Return a property from the properties file. This file is optional, so
    ...
    /**
     * For memory efficiency and performance, it's best to init NIO buffers once
     * and reuse, particularly for calls that may be repeated often. Allocate
     * NIO buffers here, use them in getSetingInt(), getModelviewMatrix(), etc.
     */
    ...
    public static boolean initDisplay() {
        ...
        try {
            if (inititalDisplayMode.trim().equalsIgnoreCase("autodetect")) {
                if ((DM = getDisplayMode(displayWidth, displayHeight, displayColorBits, displayFrequency)) == null) {
                    ...
                    if ((DM = getDisplayMode(1680, 1050, 32, 60)) == null) {
                ...
                String[] s = inititalDisplayMode.split("[ @xHz]+");
                DM = getDisplayMode(Integer.parseInt(s[0]), Integer.parseInt(s[1]), Integer.parseInt(s[2]), Integer.parseInt(s[3]));
            }
     ...
     * <P>
     * Set Mouse.setGrabbed(true). This hides the native cursor and enables the
     * Mouse.getDX() and Mouse.getDY() functions so we can track mouse motion
     * ourselves.
     *
    ...
    /**
     * Find a power of two big enough to hold the given dimension. Ie. to make a
     * texture big enough to hold a screen image for an 800x600 screen
     ...
     * one-to-one relation between screen pixel and rendered pixel, ie. if you
     * draw a 10x10 quad at 100,100 it will appear as a 10x10 pixel square on
     * screen at pixel position 100,100.
     * <P>
    ...
    /**
     * Load an image from the given file and return a ByteBuffer containing RGBA
     * pixels.<BR>
    ...
    /**
     * Load an image from the given file and return an IntBuffer containing ARGB
     * pixels.<BR>
    ...
    /**
     * Draw an image at the given x,y. If scale values are not 1, then scale the
     * image. See loadImage().<BR>
     */
    ...
    /**
     * Draw an image at the given x,y. Scale the image to the given w,h. The
     * image must be loaded into the texture pointed to by textureHandle. It
     * will be drawn in 2D on top of the current scene.
     * <BR>
     *
     * @ee loadImage().
     */
     ...
     * My cursor motion doesn't work with native cursor since verion .95 I'll
     * have to fix someday. I prefer drawing cursor as a quad anyway, so this is
     * low priority.
     *
     * @param imgFilename
     * @return Cursor
     ...
     * get flicker as front and back buffers have different images). This is an
     * issue only when you're not clearing the screen before drawing each frame,
     * ie. when leaving trails as objects move.
     */
     ...
     * stored in the given texture at 0,0 (see frameSave()) and has the same
     ...
     *
     * @param o
     * @param textureHandle / public static void renderMesh(idx3d_Object o, int
     * textureHandle) { GL11.glBindTexture(GL11.GL_TEXTURE_2D,textureHandle);
     * renderMesh(o); }
     *
     * / **
     * Basic mesh render function, using current color, material and texture.
     * @param o / public static void renderMesh(idx3d_Object o) { idx3d_Triangle
     * t; GL11.glBegin(GL11.GL_TRIANGLES); for (int j = 0; j <
     * o.triangle.length; j++) { // draw all triangles in object t =
     * o.triangle[j];
     *
     * GL11.glTexCoord2f(tileFactorHoriz*t.p1.u, tileFactorVert*t.p1.v);
     * GL11.glNormal3f(t.p1.n.x, t.p1.n.y, t.p1.n.z); GL11.glVertex3f(
     * (float)t.p1.pos.x, (float)t.p1.pos.y, (float)t.p1.pos.z);
     *
     * GL11.glTexCoord2f(tileFactorHoriz*t.p2.u, tileFactorVert*t.p2.v);
     * GL11.glNormal3f(t.p2.n.x, t.p2.n.y, t.p2.n.z); GL11.glVertex3f(
     * (float)t.p2.pos.x, (float)t.p2.pos.y, (float)t.p2.pos.z);
     *
     * GL11.glTexCoord2f(tileFactorHoriz*t.p3.u, tileFactorVert*t.p3.v);
     * GL11.glNormal3f(t.p3.n.x, t.p3.n.y, t.p3.n.z); GL11.glVertex3f(
     * (float)t.p3.pos.x, (float)t.p3.pos.y, (float)t.p3.pos.z); } GL11.glEnd();
     * }
     *
     *
     * / **
     * Render OBJ mesh (may have multiple texture coords per vert!!??!!) @param
     * o / public static void renderMeshOBJ(idx3d_Object o, int textureHandle) {
     * idx3d_Triangle t; GL11.glBindTexture(GL11.GL_TEXTURE_2D,textureHandle);
     * GL11.glBegin(GL11.GL_TRIANGLES); for (int j = 0; j < o.triangle.length;
     * j++) { // draw all triangles in object t = o.triangle[j];
     *
     ...
     * (float)t.p2.pos.x, (float)t.p2.pos.y, (float)t.p2.pos.z);
     *
     * GL11.glTexCoord2f(tileFactorHoriz*t.uvw3.x, tileFactorVert*t.uvw3.
     * y
     * );
     ...
     * do
     * {
     * while (tri[i].dist>m) i++; while (tri[j].dist<m) j--;
     *
     * if (i<=j)
     * {
     * temp=tri[i];
...
***********
     ...
     * @param o
     */
    public static void renderSameCube() {
        GL11.glBegin(GL11.GL_QUADS);
        // Front Face
    ...
    public static void renderDiffCube(int t1) {
        renderDiffCube(t1, t1, t1, t1, t1, t1);
    }

    public static void renderDiffCube(int t1, int t2, int t3, int t4, int t5, int t6) {

        // Front Face
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, t1);
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glNormal3f(0.0f, 0.0f, 1.0f);
        GL11.glTexCoord2f(0.0f, 0.0f);
        GL11.glVertex3f(-1.0f, -1.0f, 1.0f);	// Bottom Left Of The Texture and Quad
        ...
        GL11.glEnd();
        // Back Face
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, t2);
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glNormal3f(0.0f, 0.0f, -1.0f);
        GL11.glTexCoord2f(0.0f, 1.0f);
        GL11.glVertex3f(-1.0f, -1.0f, -1.0f);	// Bottom Right Of The Texture and Quad
        ...
        GL11.glEnd();
        // Top Face
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, t3);
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glNormal3f(0.0f, 1.0f, 0.0f);
        GL11.glTexCoord2f(1.0f, 0.0f);
        GL11.glVertex3f(-1.0f, 1.0f, -1.0f);	// Top Left Of The Texture and Quad
        ...
        GL11.glEnd();
        // Bottom Face
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, t4);
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glNormal3f(0.0f, -1.0f, 0.0f);
        GL11.glTexCoord2f(0.0f, 0.0f);
        GL11.glVertex3f(-1.0f, -1.0f, -1.0f);	// Top Right Of The Texture and Quad
        ...
        GL11.glEnd();
        // Right face
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, t5);
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glNormal3f(1.0f, 0.0f, 0.0f);
        GL11.glTexCoord2f(0.0f, 0.0f);
        GL11.glVertex3f(1.0f, -1.0f, -1.0f);	// Bottom Right Of The Texture and Quad
        ...
        // Left Face
        GL11.glEnd();
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, t6);
        GL11.glBegin(GL11.GL_QUADS);
        GL11.glNormal3f(-1.0f, 0.0f, 0.0f);
        GL11.glTexCoord2f(1.0f, 0.0f);
        GL11.glVertex3f(-1.0f, -1.0f, -1.0f);	// Bottom Left Of The Texture and Quad
    ...
    /**
     * Render a unit sphere at the origin, using current color, texture and
     * material. This function calls makeSphere() to create the sphere in a
     * display list.
     */
    ...
    /**
     * Create a Display List of a sphere. Sphere will be at origin, will have
     * radius of 1 unit, and will have 24 segments x 24 slices.
     * <P>
     * The LWJGL sphere has it's poles aligned horizontally. I rotate 90 degrees
     * around the X axis to get the poles of the sphere vertical. Just a
     * preference, not critical.
     * <P>
     * Display Lists store opengl rendering commands. In this example the opengl
     * commands to render a sphere are stored into a display list for future
     * drawing. This approach is often used to increase performance.
     * <P>
    ...
    /**
     * Save the contents of the current render buffer to a BUFFEREDIMAGE(for jpg
     * argb) image. If the current buffer is the framebuffer then this will work
     * as a screen capture. Can also be used with the PBuffer class to copy
     * large images or textures that have been rendered into the offscreen
     * pbuffer.
     * <P>
    ...
    /**
     * Save the contents of the current render buffer to a PNG (argb)image. If
     * the current buffer is the framebuffer then this will work as a screen
